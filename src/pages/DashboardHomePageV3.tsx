...
const normalizeTeamId = (id: string | undefined | null) => {
    if (!id) return '';
    // Handle "TEAM_5", "TEAM_05", etc.
    const teamMatch = id.match(/TEAM_(\d+)/i);
    if (teamMatch) return `TEAM_${parseInt(teamMatch[1])}`;

    // Handle "5팀", "5 팀", "제5팀"
    const krMatch = id.match(/(\d+)\s*팀/);
    if (krMatch) return `TEAM_${parseInt(krMatch[1])}`;

    return id.trim().toUpperCase();
};

export default function DashboardHomePageV3() {
    ...
    const tabs = useMemo(() => [
        ...
        ...teams.filter(t => t.TeamID.startsWith('TEAM_')).map(t => ({ id: t.TeamID, name: t.TeamName }))
    ], [teams]);
    ...
    const teamSummaryData = useMemo(() => {
        ...
        return teams.filter(t => t.TeamID.startsWith('TEAM_')).map(t => {
            ...
            if (isSalesTeam) {
                ...
                return { name: t.TeamName, id: t.TeamID, revenue, grossRevenue, expense: labor + outsource + other, profit: grossRevenue - (labor + outsource + other), labor, outsource, other, supportRevenue: supportDeduction };
                ...
                return { name: t.TeamName, id: t.TeamID, revenue, grossRevenue, expense: labor + outsource + other, profit: grossRevenue - (labor + outsource + other), labor, outsource, other, supportRevenue: revenue };
            }
        });
    ...
    const divisionTotal = useMemo(() => {
        ...
        const teamExpenseSum = teamSummaryData.reduce((sum, t) => sum + t.expense, 0);
        const totalExpense = teamExpenseSum + commonProjectExpenses;

        return { name: '철도2부 계', revenue, expense: totalExpense, profit: revenue - totalExpense };
    }, [teamSummaryData, costExpenses, projects, startMonth, endMonth]);
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-6 gap-6">
                ...
                <div className="flex flex-col md:flex-row items-start md:items-center space-y-4 md:space-y-0 md:space-x-6 w-full lg:w-auto">
                    <h1 className="text-xl md:text-2xl font-black tracking-tighter text-[#004442] border-l-[6px] md:border-l-[8px] border-[#004442] pl-3 md:pl-4 leading-none">
                        {t.navDashboard} <span className="text-base text-pink-600 align-top ml-1">V3</span>
                    </h1>
                    <p className="text-[10px] text-slate-400 mt-1 font-black uppercase tracking-[0.2em] pl-3 md:pl-4 italic">Railway Division 2 Intelligence Hub</p>
                                        ...
                                        <div>
                                            ...
                                            <CardContent className="p-4 md:p-5 pt-1">
                                                <div className="space-y-3 md:space-y-4">
                                                    ...
                                                    <div className="flex justify-between items-center pt-2 border-t border-slate-50 overflow-hidden">
                                                        <div className="flex flex-col">
                                                            <span className="text-slate-500 font-bold text-[11px] uppercase leading-tight">결산액</span>
                                                        </div>
                                                        <p className={cn("text-lg md:text-2xl font-black leading-none whitespace-nowrap", team.profit >= 0 ? "text-emerald-600" : "text-rose-600")}>
                                                    ...
                                                    <div className="flex justify-end">
                                                        <div className={cn(
                                                            ...
                                                            {team.revenue > 0 ? ((team.profit / team.revenue) * 100).toLocaleString(undefined, { maximumFractionDigits: 0 }) : 0}%
                                                        </div>
                                                    </div>
                                                </div>
                                            </CardContent>
                                    ...
                                    <CardContent className="p-4 md:p-5 pt-1">
                                        <div className="space-y-4">
                                            ...
                                            <div className="flex justify-between items-center border-t border-white/10 pt-4 mt-2">
                                                <div className="flex flex-col">
                                                    <span className="text-indigo-100 font-bold text-xs uppercase leading-tight">결산액</span>
                                                </div>
                                                <p className="text-xl md:text-3xl font-black text-white">₩{(divisionTotal.profit / 1000000).toLocaleString(undefined, { maximumFractionDigits: 0 })}M</p>
                                            </div>
                                        </div>
                                    </CardContent>
                                </div>
                                    ...
                                    <table className="w-full text-sm text-left border-collapse">
                                        <thead>
                                            <tr className="bg-slate-50 text-slate-500 text-base font-black uppercase tracking-widest border-b">
                                                ...
                                                <th className="px-8 py-8 text-right border-l border-slate-100 text-lg">{t.netProfit}</th>
                                            </tr>
                                        </thead>
                                        <tbody>
